...
def main():
    ...
    outdir="figs_rela_rank_percentage_TFs"
    ...
    for thre in thres:
        for ii in np.arange(1):
            ...
            for key in keys:
                ...
                data_coverage = df.loc[notNull_index].dropna().groupby(['TF']).min().index
                data_coverage_true = df_true.loc[notNull_index].dropna().groupby(['TF']).min().index
                
                compr_df.loc[key,'tf_coverage'] = len(data_coverage)
                compr_df.loc[key,'tf_coverage_true'] = len(data_coverage_true)
                for bar_pos in np.arange(len(top_thres)):
                    top_thre = top_thres[bar_pos]
                    top_val = len((df_true.loc[df_true['Final_rank']/tf_total<top_thre]).loc[notNull_index].dropna().groupby(['TF']).min().index)
                    compr_df.loc[key,'true_top{}'.format(top_thre)] = top_val
                    # plot bars
                    if top_thre==0.005:
                        color = colors[bar_pos]
                        color = lighten_color(method_match_colors[key],1.23)
            ...
            plt.ylabel('Number of TRs',fontsize=17)
