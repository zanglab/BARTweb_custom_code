...
def main():
    ...
    results_sub_dirs = ['f3_deg_homer/homer_deg_FC1.5']
    method_name="HOMER"
    ...
    for ii in np.arange(2):
        ...
        for gene_list in gene_lists[:]:
            ...
            up_result_file = results_dir+os.sep+results_sub_dirs[ii]+os.sep+'{}_UpGenes/knownResults.txt'.format(gene_list)
            if not os.path.isfile(up_result_file):
                up_result_file = results_dir+os.sep+results_sub_dirs[ii]+"_append"+os.sep+'{}_UpGenes/knownResults.txt'.format(gene_list)
            dn_result_file = results_dir+os.sep+results_sub_dirs[ii]+os.sep+'{}_DnGenes/knownResults.txt'.format(gene_list)
            if not os.path.isfile(dn_result_file):
                dn_result_file = results_dir+os.sep+results_sub_dirs[ii]+"_append"+os.sep+'{}_DnGenes/knownResults.txt'.format(gene_list)
            
            if os.path.isfile(up_result_file):
                up_df = pd.read_csv(up_result_file,sep='\t',index_col=0)
                rank = return_index_rank(tf_name,up_df.index)
                out_df.loc[gene_list,'Up_rank'] = rank
                out_df.loc[gene_list,'Up_total'] = len(up_df.index)
                if rank !=-1:
                    out_df.loc[gene_list,'Up_pvalue'] = up_df.iloc[rank-1]['P-value']
#                 out_df.loc[gene_list,'Up_pvalue'] = up_df.iloc[rank-1]['q-value (Benjamini)']
            if os.path.isfile(dn_result_file):
                dn_df = pd.read_csv(dn_result_file,sep='\t',index_col=0)
                rank = return_index_rank(tf_name,dn_df.index)
                out_df.loc[gene_list,'Dn_rank'] = rank
                out_df.loc[gene_list,'Dn_total'] = len(dn_df.index)
                if rank !=-1:
                    out_df.loc[gene_list,'Dn_pvalue'] = dn_df.iloc[rank-1]['P-value']
#                 out_df.loc[gene_list,'Dn_pvalue'] = dn_df.iloc[rank-1]['q-value (Benjamini)']
