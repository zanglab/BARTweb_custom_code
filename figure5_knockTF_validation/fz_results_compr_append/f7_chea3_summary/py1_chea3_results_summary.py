...
def main():
    ...
    results_sub_dirs = ['f7_ChEA3/chea3_deg_FC1.5']
    method_name="CHEA3"
    ...
    for ii in np.arange(2):
        ...
        for gene_list in gene_lists[:]:
            ...
            if os.path.isfile(up_result_file):
                up_df = pd.read_csv(up_result_file,sep=',',index_col=1)#;print(up_df);exit()
                if tf_name in up_df.index:
                    out_df.loc[gene_list,'Up_rank'] = list(up_df.index).index(tf_name)+1
                    out_df.loc[gene_list,'Up_pvalue'] = up_df.iloc[list(up_df.index).index(tf_name)]['Pvalue']
#                 out_df.loc[gene_list,'Up_rank'] = return_index_rank(tf_name,up_df.index)
            ...
            if os.path.isfile(dn_result_file):
                ...
                if tf_name in dn_df.index:
                    ...
                    out_df.loc[gene_list,'Dn_pvalue'] = dn_df.iloc[list(dn_df.index).index(tf_name)]['Pvalue']
